...
{/*let url = `http://localhost:3030/api/create-order-id`;
let response = await fetch(url, { amount : totalPrices });
let data = await response.json();
//let {order} = data
console.log(response);*/}
...
var options = {
  key: "rzp_test_RB0WElnRLezVJ5", // Enter the Key ID generated from the Dashboard
   ...
   currency : order.currency, // Amount is in currency subunits. Default currency is INR. Hence, 50000 refers to 50000 paise

  //currency: "INR",
  name: "Zomato Clone",
  description: "Make payment to get order",
  image:
  ...
  //handler: async function (response) {
   ...
   let url = `http://localhost:3030/api/verify-paymant`;

  let {data} = await axios.post(url,sendData);
if(data.status===true){
  alert("Payment done successflly, order saved");
                      ...
                      {rDetails.thumb.map((value, index) => {
                        return (
                          <div key={index} className="w-100">
                            <img style={{ height: "75vh " }}  src={"/images/" + value} />
                          </div>
                        );
                      })}
                    </Carousel>
